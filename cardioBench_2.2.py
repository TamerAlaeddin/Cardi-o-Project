...
class MainWindow:
    ...
    def threading(self, start, duration, control):
        ...
        # make window larger
        self.set_screen(1000, 900)

        resFile = glob.glob(self.cardioFeed_selection + "/Res*.csv")
        self.cf_data = pd.read_csv(resFile[0])
    ...
    def append_to_list(self, string, control):
        ...
        # shows an error cardioFeed output does not start with "T "
        if x[0] != 'T':
    ...
    def generate_excel(self, df):
        ...
        messagebox.showinfo("Success",
                            f"{os.path.basename(self.cardioFeed_selection) + '_output.xlsx'} generated successfully!")
    ...
    def get_df(self):
        ...
        for index, row in data.iterrows():
            # get cardio_hr and rr matching original data index
            ...
            # append cardio_hr and rr with original data to new list
            merged_data.append([
                row['time(s)'],
                row['HR'],
                row['RR'],
    ...
    def cf_metrics(self, df, lower, upper):
        ...
        return metrics, cardio_std, patient_std
    ...
    def cf_to_pdf(self, fig1, fig2, fig3):
        ...
        # get metrics
        metrics, cardio_std, patient_std = self.cf_metrics(df, start_time, end_time)

        cardio_accuracy_upper = metrics['cardio error'].abs().mean() + (3 * cardio_std)
        cardio_accuracy_lower = metrics['cardio error'].abs().mean() - (3 * cardio_std)
        patient_accuracy_upper = metrics['patient error'].abs().mean() + (3 * patient_std)
        patient_accuracy_lower = metrics['patient error'].abs().mean() - (3 * patient_std)
        ...
        table_data = [
            ...
            ['Cardio', f"{metrics['cardio error'].abs().mean():.2f}", f"{cardio_std:.2f}",
             f"{cardio_accuracy_upper:.2f} / {cardio_accuracy_lower:.2f}", self.saved_cardio_threshold],
            ['Patient', f"{metrics['patient error'].abs().mean():.2f}", f"{patient_std:.2f}",
             f"{patient_accuracy_upper:.2f} / {patient_accuracy_lower:.2f}", self.saved_patient_threshold]
        ]
    ...
    def next_gui(self, control):
        ...
        self.b3, self.b3l = create_cell(self.tableframe, f"{cardio_std:.2f}", 1, 2)
        ...
        prev_cardio = self.read_config('cardio_threshold')
        cardio_threshold.set(prev_cardio)
        ...
        self.c3, self.c3l = create_cell(self.tableframe, f"{patient_std:.2f}", 2, 2)
        ...
        prev_patient = self.read_config('patient_threshold')
        patient_threshold.set(prev_patient)
        ...
        self.printtoPDF = CTkButton(self.sub_bottomframe, text="Print to PDF",
                                    command=lambda: self.cf_to_pdf(fig, fig2, fig3),
                                    font=("Arial", 16), fg_color="#1976D2", hover_color="#424242", corner_radius=5)
        self.printtoPDF.grid(row=0, column=0, padx=5)

        self.generateExcel = CTkButton(self.sub_bottomframe, text="Generate Excel File",
                                       command=lambda: self.generate_excel(df),
                                       font=("Arial", 16), fg_color="#1976D2", hover_color="#424242", corner_radius=5)
    ...
    def check_queue(self, control):
        try:
            ...
            index = int(output.split()[1])
            start_time = self.output_list[1][0]
            ...
            hr_plt.plot(T, HR, label='Cardio-HR')
            hr_plt.plot(T, self.cf_data['HR'].loc[start_time:index], label='Patient-HR')
            hr_plt.plot(T, self.cf_data['SpO2HR'].loc[start_time:index], label='SpO2-HR')
            hr_plt.legend()
            hr_plt.set_ylim([0, None])
            hr_plt.set_xlim([0, None])
            hr_plt.set_xlabel('Time (seconds)')
            ...
            rr_plt.plot(T, RR, label='Cardio-RR')
            rr_plt.plot(T, self.cf_data['RR'].loc[start_time:index], label='Patient-RR')
            rr_plt.legend()
            rr_plt.set_ylim([0, None])
            rr_plt.set_xlim([0, None])
            rr_plt.set_xlabel('Time (seconds)')
            ...
            # display heart rate plot
            self.q_graph = canvas.get_tk_widget()
            self.q_graph.pack(side=TOP, fill=BOTH, expand=1)

            # display respiratory rate plot
            self.q_graph2 = canvas2.get_tk_widget()
            self.q_graph2.pack(side=TOP, fill=BOTH, expand=1)
        ...
        elif self.err_control == 1:  # if there is an error, return to cardioFeed gui
            # note that this does not stop program execution to allow for cardioFeed to stop properly

            # delete the raw file
            self.deleteCSV()
            # navigate back to main directory
            os.chdir('..')
            # return to cardioFeed gui
            self.cardioFeed_gui(control)
        else:
    ...
    def cardioFeed_gui(self, control):
        ...
        # reset output_list and error control so cardioFeed can be run again
        self.output_list = [["T", "HR", "RR", "M", "S"]]
        self.err_control = 0
        ...
        # checkbox to create .csv file from output
        # excelVar = IntVar()
    ...
    def update_config(self):
        with open('config.csv', 'w', newline='') as csvfile:
            ...
            writer.writerow([self.patientDirectory, self.saved_cardio_threshold, self.saved_patient_threshold])

    def read_config(self, value_to_read):
        with open('config.csv', newline='') as csvfile:
            reader = csv.reader(csvfile)
            for row in reader:
                if value_to_read == 'directory':
                    return row[0]
                elif value_to_read == 'cardio_threshold':
                    return row[1]
                elif value_to_read == 'patient_threshold':
                    return row[2]
                else:
                    return ''
    ...
    def root_gui(self):
        ...
        self.saved_cardio_threshold = float(self.read_config('cardio_threshold'))
        self.saved_patient_threshold = float(self.read_config('patient_threshold'))
        print(self.saved_patient_threshold, self.saved_cardio_threshold)
